...
slug="$(vault_slugify_tech_id "$base_no_ext" "docx")"
...
if [[ "${create_task}" == "1" && ! -f "$task_path" ]]; then
  {
    ...
    echo "---"
    echo
    echo "# Изучить DOCX и сделать разбор"
    echo
    echo "## Next step"
    echo "- [ ] Изучить DOCX: ${note_link}"
    echo
    echo "- [ ] Подготовить разбор DOCX и список уточнений по ${note_link}"
    echo
    echo "## Checklist"
    echo "- [ ] Зафиксировать summary / обязательства / сроки"
    echo "- [ ] Подготовить вопросы/риски для уточнения"
    echo
    echo "## Context / Links"
    echo "- Note: ${note_link}"
    if [[ -n "${project_link}" ]]; then
